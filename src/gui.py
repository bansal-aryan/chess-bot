...
from src.bot import find_best_move 
...
class ChessWindow(QMainWindow):
    def __init__(self):
        ...
        page_layout.addWidget(self.claim_draw_button)
    ...
    def refresh_board(self):
        self.claim_draw_button.setEnabled(
            not self.draw_claimed
            and not self.board.is_game_over()
            and self.board.turn == chess.WHITE
            and self.board.can_claim_draw()
        )
    ...
    def handle_square_click(self, square):
        if (self.draw_claimed or self.board.is_game_over()):
        ...
        promotions = [
            candidate for candidate in self.board.legal_moves
            if candidate.from_square == self.selected_square
            and candidate.to_square == square
        ...
        if promotions:
            ...
            choices = [names[candidate.promotion] for candidate in promotions]
            choice, accepted = QInputDialog.getItem(
                self, "Promote pawn", "Promote to:", choices, 0, False
            )
            if not accepted:
                return
            move = promotions[choices.index(choice)]
    ...
    def claim_draw(self):
        if (self.draw_claimed or self.board.is_game_over()
                or self.board.turn != chess.WHITE
    ...
    def update_status(self, move_name):
        if self.draw_claimed:
            self.status_label.setText("Game over: 1/2-1/2 (draw claimed)")
            return
    ...
    def make_bot_move(self):
        ...
        if move is None or move not in self.board.legal_moves:
            self.status_label.setText("Bot could not find a legal move")
            return
        move_name = self.board.san(move)
        self.board.push(move)
